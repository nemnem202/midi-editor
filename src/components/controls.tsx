...
import { ListMusic, Play, Plus, Square } from "lucide-react";
...
export default function ControlsPannel() {
  ...
  return (
    <div className="flex gap-2 w-fit h-fit p-2 bg-card rounded-xl items-center select-none">
      ...
      <Field className="flex flex-row items-center justify-center !w-min">
        ...
        <Input
          type="number"
          value={project.config.signature[0]}
          className="!w-10 min-w-0 p-0 text-center"
        />
        <span className="!w-min">/</span>
        <Input
          type="number"
          value={project.config.signature[1]}
          className="!w-10 min-w-0 p-0 text-center"
        />
      </Field>
      <Separator orientation="vertical" className="!h-6" />
